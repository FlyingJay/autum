 ...
 * returns {undefined}
 */
const transform = (element, coordinateX, coordinateY, rot) => {
  element.style[vendorPrefix('transform')] = 'translate3d(0, 0, 0) translate(' + coordinateX + 'px, ' + coordinateY + 'px) rotate(' + rot + 'deg)';
};
 ...
 * returns {boolean}
 */
const isThrowOut = (xOffset, yOffset, element, throwOutConf) => {
  return throwOutConf === 1;
};
...
const Card = (stack, targetElement) => {
  ...
  const construct = () => {
    ...
    throwOutDist = config.throwOutDistance(config.minThrowOutDistance, config.maxThrowOutDistance);
    ...
    eventEmitter.on('panend', (event) => {
      ...
      const throwOut = config.isThrowOut(
        coordinateX,
        coordinateY,
        targetElement,
      ...
      if (throwOut && direction !== Direction.INVALID) {
    ...
    springThrowOut.addListener({
      ...
      onSpringUpdate: (spring) => {
        ...
        if (lastThrow.direction === Direction.RIGHT || lastThrow.direction === Direction.LEFT) {
          ...
          coordianteX = rebound.util.mapValueInRange(value, 0, 1, lastThrow.fromX, throwOutDist * directionFactor);
          ...
          coordianteY = rebound.util.mapValueInRange(value, 0, 1, lastThrow.fromY, throwOutDist * directionFactor);
        }
    ...
    doMove = () => {
      ...
      const rot = config.rotation(coordinateX, coordianteY, targetElement, config.maxRotation);

      config.transform(targetElement, coordinateX, coordianteY, rot);
    ...
    onSpringUpdate = (coordinateX, coordinateY) => {
      const rot = config.rotation(coordinateX, coordinateY, targetElement, config.maxRotation);

      lastTranslate.coordinateX = coordinateX || 0;
      lastTranslate.coordinateY = coordinateY || 0;

      config.transform(targetElement, coordinateX, coordinateY, rot);
    };
